...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Prisma client şeması gerideyse (Unknown argument), ilgili alanı düşürüp retry et.
    const blockedFields = new Set<string>()
    let financialData
    while (true) {
      const filteredFields = Object.fromEntries(
        Object.entries(financialFields).filter(([k]) => !blockedFields.has(k))
      )

      try {
        financialData = await prisma.financialData.upsert({
          where: { entityId_year_period: { entityId, year, period } },
          update: { ...filteredFields, source, updatedAt: new Date() },
          create: { entityId, year, period, source, ...filteredFields },
        })
        break
      } catch (error) {
        ...
        if (!isUnknownArgumentError || !unknownArg) throw error
        if (blockedFields.has(unknownArg)) throw error
        if (!(unknownArg in financialFields)) throw error
