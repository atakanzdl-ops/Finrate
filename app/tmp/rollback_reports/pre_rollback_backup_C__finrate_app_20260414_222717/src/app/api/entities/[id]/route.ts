...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // DB semasi geride kaldiginda (eksik kolon), endpoint JSON donmeye devam etsin.
    if (error?.code === 'P2022') {
      entity = await prisma.entity.findFirst({
        where: { id, userId },
      ...
      return jsonUtf8({ error: 'Sunucu hatasi.' }, { status: 500 })
    }
  }

  if (!entity) return jsonUtf8({ error: 'Bulunamadi.' }, { status: 404 })
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!existing) return jsonUtf8({ error: 'Bulunamadi.' }, { status: 404 })
