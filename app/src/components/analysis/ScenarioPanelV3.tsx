  ...
  return (
    <div className="space-y-6">
      ...
      */}
      ...
      {/* E. TARGET PACKAGE META DEBUG (R8.4.3) — selectTargetPackage çıktısı */}
      {da.targetPackageMeta && (() => {
        const meta = da.targetPackageMeta
        const statusColor: Record<string, string> = {
          REACHED:       'bg-green-50 border-green-200 text-green-800',
          NOT_REACHED:   'bg-amber-50 border-amber-200 text-amber-800',
          FALLBACK:      'bg-blue-50 border-blue-200 text-blue-800',
          SOURCE_MISMATCH: 'bg-purple-50 border-purple-200 text-purple-800',
        }
        const badgeClass = statusColor[meta.status] ?? 'bg-slate-50 border-slate-200 text-slate-800'
        ...
        return (
          ...
          >
            <h3
              className="text-sm font-bold text-[#0B3C5D] flex items-center gap-2 mb-4"
              style={{ fontFamily: 'Outfit, sans-serif' }}
            >
              <span className="opacity-60">⚙</span>
              Paket Seçim Özeti (R8.4.3)
            </h3>
            <div className="flex flex-wrap gap-3 mb-4">
              ...
              {meta.fallback && (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border bg-slate-50 border-slate-200 text-slate-600">
                  fallback
                </span>
              )}
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div className="bg-slate-50 rounded-[8px] p-3">
                <div className="text-xs text-[#64748B] mb-1">Seçilen (ham satır)</div>
                <div className="font-semibold text-[#1E293B]">{meta.rawSelectedActionCount ?? meta.selectedActionCount ?? '—'}</div>
              </div>
              <div className="bg-slate-50 rounded-[8px] p-3">
                <div className="text-xs text-[#64748B] mb-1">Seçilen (görünen kart)</div>
                <div className="font-semibold text-[#1E293B]">{meta.displayActionCount ?? '—'}</div>
              </div>
              <div className="bg-slate-50 rounded-[8px] p-3">
                <div className="text-xs text-[#64748B] mb-1">Seçilen paket tutarı</div>
                <div className="font-semibold text-[#1E293B] text-xs">{meta.selectedPackageAmountTRY != null ? formatTRY(meta.selectedPackageAmountTRY) : '—'}</div>
              </div>
              <div className="bg-slate-50 rounded-[8px] p-3">
                <div className="text-xs text-[#64748B] mb-1">Tam portföy tutarı</div>
                <div className="font-semibold text-[#1E293B] text-xs">{meta.fullPortfolioAmountTRY != null ? formatTRY(meta.fullPortfolioAmountTRY) : '—'}</div>
              </div>
            </div>
            {meta.warnings?.length > 0 && (
              <div className="mt-4 space-y-1">
                <div className="text-xs font-medium text-[#64748B] uppercase tracking-wide mb-2">Guard Uyarıları</div>
                {meta.warnings.map((w: string, i: number) => (
                  <div key={i} className="text-xs text-amber-800 bg-amber-50 border border-amber-100 rounded px-3 py-2">
                    {w}
                  </div>
                ))}
              </div>
