...
exports.parseExcelNumber = parseExcelNumber;
exports.parseMizanRows = parseMizanRows;
...
function parseExcelNumber(raw) {
    ...
    if (['#SAYI0!', '#DIV/0!', '#NUM!', '#DEGER!', '#VALUE!'].includes(upper))
        return null;
...
// ─── Dikey format ─────────────────────────────────────────────────────────────
function parseYearHeaderCell(raw) {
    if (raw == null)
        return null;
...
function isCodeHeaderCell(raw) {
    const n = norm(raw);
    return n.includes('hesap kod') || n === 'kod' || n.includes('hesap kodu');
}
function findVerticalYearHeader(rows) {
    ...
    for (let i = 0; i < scan; i++) {
        ...
        for (let j = 0; j < row.length; j++) {
            ...
            if (parsed)
                yearCols.push({ colIdx: j, year: parsed.year, period: parsed.period });
        }
...
function detectVerticalFormat(rows) {
    if (rows.length < 3)
        return false;
...
function parseVerticalExcel(rows) {
...
function detectMizanFormat(rows) {
    for (let i = 0; i < Math.min(12, rows.length); i++) {
        const row = rows[i];
        if (row.some(c => isCodeHeaderCell(c)))
            return true;
    }
    return false;
}
function findMizanHeader(rows) {
    for (let i = 0; i < Math.min(12, rows.length); i++) {
        const row = rows[i];
        if (!row.some(c => isCodeHeaderCell(c)))
            continue;
        ...
        row.forEach((cell, idx) => {
            const n = norm(cell);
            if (isCodeHeaderCell(cell)) {
                cols['code'] = idx;
                return;
            }
            // bakBorc: hücre hem "bak" hem "bor" içeriyorsa
            if (n.includes('bak') && n.includes('bor')) {
                cols['bakBorc'] = idx;
                return;
            }
            if (n.includes('bak') && n.includes('alacak')) {
                cols['bakAlacak'] = idx;
                return;
            }
            // sadece bor veya alacak (bakiyesiz sütunlar)
            if (n.includes('bor') && cols['borc'] === undefined) {
                cols['borc'] = idx;
            }
            if (n.includes('alacak') && cols['alacak'] === undefined) {
                cols['alacak'] = idx;
            }
        });
        console.log('[HEADER] row', i, JSON.stringify(cols));
        if ('code' in cols && ('borc' in cols || 'bakBorc' in cols))
            return { headerIdx: i, cols };
    }
    return null;
}
function extractMizanYear(rows) {
    for (let i = 0; i < Math.min(6, rows.length); i++) {
        for (const cell of rows[i]) {
            if (!cell)
                continue;
            const m = String(cell).match(/(\d{2})[.\/-](\d{2})[.\/-](20\d{2})(?:\s*[-–—]\s*|\s+)(\d{2})[.\/-](\d{2})[.\/-](20\d{2})/);
            if (m) {
...
function parseMizanRows(rows) {
    ...
    for (let i = headerIdx + 1; i < rows.length; i++) {
        ...
        if (!nc || nc.length > 3 || nc.length < 2)
            continue;
        if (MIZAN_IGNORE.has(nc))
            continue;
        const getNum = (key, fallback) => {
            const idx = cols[key] !== undefined
                ? cols[key]
        ...
        if (MIZAN_SPLIT[nc]) {
            if (bb > 0)
                add(MIZAN_SPLIT[nc].bb, bb);
            if (ba > 0)
                add(MIZAN_SPLIT[nc].ba, ba);
            continue;
        }
        const mapped = MIZAN_MAP[nc];
        if (!mapped)
            continue;
        if (mapped.endsWith('_CA'))
            add(mapped.replace('_CA', ''), -ba);
        else if (mapped.endsWith('_CB'))
            add(mapped.replace('_CB', ''), -bb);
        else if (mapped.endsWith('_A'))
            add(mapped.replace('_A', ''), ba);
        else
            add(mapped, bb);
    }
...
function parseExcelBuffer(buffer, _fileName) {
    ...
    if (detectVerticalFormat(rows)) {
        ...
        if (!result.length)
            console.warn('[excel] vertical empty', { sheetName });
        return result;
    }
    // 2) Mizan (DETAY MİZAN)
    if (detectMizanFormat(rows)) {
        ...
        if (!result.length)
            console.warn('[excel] mizan parse empty', { sheetName, rowCount: rows.length });
        return result;
    }
    ...
    for (const row of rows.slice(1)) {
        ...
        headerRow.forEach((header, i) => {
            ...
            if (/^(yil|year)$/.test(n)) {
                year = row[i] ? Number(row[i]) : undefined;
                return;
            }
            if (/^(donem|period)$/.test(n)) {
                period = row[i] ? String(row[i]).toUpperCase() : undefined;
                return;
            }
            const mapped = COLUMN_MAP[n];
            if (mapped)
                fields[mapped] = parseExcelNumber(row[i]);
            else if (row[i] != null && n)
                unmapped.push(n);
        });
    ...
    if (!results.length)
        console.warn('[excel] horizontal empty', { sheetName });
    return results;
}
// ─── CSV ──────────────────────────────────────────────────────────────────────
