  ...
  const ratioSections = selected ? [
    ...
    },
    {
      ...
      rows: [
        ...
        { id: 'Net Borç / FAVÖK',        desc: 'Net finansal borç / FAVÖK. Mevcut FAVÖK ile borcun kaç yılda ödenebileceğini gösterir. 3.0x altı iyi, 5.0x üzeri kritik sayılır. Net nakit pozisyonunda (nakit > finansal borç) uygulanmaz.', val: r.debtToEbitda == null ? '—' : r.debtToEbitda < 0 ? 'Net nakit' : r.debtToEbitda >= 99 ? '> 99x (FAVÖK ≤ 0)' : fmtN(r.debtToEbitda, 1) + 'x', avg: fmtN(bm.debtToEbitda, 1) + 'x', good: r.debtToEbitda != null && r.debtToEbitda <= 3 },
        { id: 'Faiz Karşılama',         desc: 'FVÖK / faiz giderleri. Faiz ödemelerinin kaç katı FVÖK üretildiğini gösterir. 2.0x altı riskli, 4.0x üzeri rahat kabul edilir. Faiz gideri yoksa uygulanamaz.',                val: r.interestCoverage == null ? '—' : r.interestCoverage >= 9999 ? 'Uygulanamaz (faiz gideri yok)' : fmtN(r.interestCoverage, 1) + 'x', avg: fmtN(bm.interestCoverage, 1) + 'x', good: r.interestCoverage != null && (r.interestCoverage >= 9999 || r.interestCoverage >= bm.interestCoverage * 0.8) },
      ]
    },
    {
