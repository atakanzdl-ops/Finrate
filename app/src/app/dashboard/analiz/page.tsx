...
function AnalizPageContent() {
  ...
  // Rasyo tablosu ikonu: aynı kural — "iyi" veya "uygulanamaz" → olumlu
  const ok = (key: Parameters<typeof assess>[0], v: number | null | undefined, bmv: number | null | undefined) => {
  ...
  const ratioSections = selected ? [
    {
      ...
      rows: [
        { id: 'Cari Oran',              desc: 'Dönen varlıklar / kısa vadeli borçlar. Firmanın kısa vadeli yükümlülüklerini dönen varlıklarıyla ne ölçüde karşılayabildiğini gösterir. 1.5x üzeri sağlıklıdır.',             val: fmtN(r.currentRatio),             avg: fmtN(bm.currentRatio),              good: ok('currentRatio', r.currentRatio, bm.currentRatio) },
        { id: 'Asit-Test (Hızlı)',      desc: '(Dönen varlıklar − Stoklar) / kısa vadeli borçlar. Stok satışına gerek kalmadan anlık yükümlülükleri karşılama gücü. 1.0x üzeri iyi sayılır.',                               val: fmtN(r.quickRatio),               avg: fmtN(bm.quickRatio),                good: ok('quickRatio', r.quickRatio, bm.quickRatio) },
        { id: 'Nakit Oran',             desc: 'Nakit & benzerleri / kısa vadeli borçlar. En katı likidite ölçüsüdür; yalnızca eldeki nakitle borç karşılanabilirliğini gösterir.',                                           val: fmtN(r.cashRatio),                avg: fmtN(bm.cashRatio),                 good: ok('cashRatio', r.cashRatio, bm.cashRatio) },
        ...
        { id: 'Nakit İhtiyaç Süresi',   desc: 'Stok devir + alacak tahsil − borç ödeme süresi (gün). Düşük değer daha hızlı nakit döngüsü anlamına gelir.',                                                                   val: r.cashConversionCycle != null ? fmtN(r.cashConversionCycle, 0) + ' gün' : '—', avg: fmtN(bm.cashConversionCycle, 0) + ' gün', good: ok('cashConversionCycle', r.cashConversionCycle, bm.cashConversionCycle) },
    ...
    },
    {
      ...
      rows: [
        { id: 'Brüt Kar Marjı',                   desc: '(Satışlar − SMM) / Satışlar. Üretim/tedarik maliyeti düşüldükten sonra kalan kâr payı. Yükseldikçe rekabet gücü artar.',                                          val: fmtPct(r.grossMargin),            avg: fmtPct(bm.grossMargin),             good: ok('grossMargin', r.grossMargin, bm.grossMargin) },
        { id: 'FAVÖK Marjı',                       desc: 'Faiz, vergi ve amortisman öncesi kâr / satışlar. Şirketin nakit üretme kapasitesini gösterir; borç ödeme gücünün temel göstergesidir.',                            val: fmtPct(r.ebitdaMargin),           avg: fmtPct(bm.ebitdaMargin),           good: ok('ebitdaMargin', r.ebitdaMargin, bm.ebitdaMargin) },
        ...
        { id: 'Net Kar Marjı',                     desc: 'Net kâr / satışlar. Tüm giderler, faiz ve vergi sonrası kalan kâr payı. Bankaların en çok izlediği karlılık göstergesidir.',                                       val: fmtPct(r.netProfitMargin),        avg: fmtPct(bm.netProfitMargin),        good: ok('netProfitMargin', r.netProfitMargin, bm.netProfitMargin) },
        { id: 'Aktif Karlılığı (ROA)',             desc: 'Net kâr / toplam aktif. Varlıkların ne kadar verimli kullanıldığını gösterir. %4+ iyi, %8+ çok iyi kabul edilir.',                                                 val: fmtPct(r.roa),                    avg: fmtPct(bm.roa),                     good: ok('roa', r.roa, bm.roa) },
        { id: 'Özkaynak Karlılığı (ROE)',          desc: 'Net kâr / özkaynak. Hissedar yatırımının getirisidir. %10+ iyi, sektör ortalamasının üzeri çok iyi sayılır.',                                                      val: fmtPct(r.roe),                    avg: fmtPct(bm.roe),                     good: ok('roe', r.roe, bm.roe) },
        ...
        { id: 'Nominal Gelir Büyümesi',            desc: 'Cironun yıllık büyüme oranı (enflasyon dahil). Pozitif büyüme şirketin büyüdüğüne işaret eder.',                                                                   val: r.revenueGrowth != null ? fmtPct(r.revenueGrowth) : '—', avg: fmtPct(bm.revenueGrowth), good: ok('revenueGrowth', r.revenueGrowth, bm.revenueGrowth) },
    ...
    },
    {
      ...
      rows: [
        { id: 'Aktif Devir Hızı',       desc: 'Satışlar / toplam aktif. Varlıkların ne kadar etkin kullanıldığının ölçüsüdür. Yükseldikçe operasyonel verimlilik artar.',                                                    val: fmtN(r.assetTurnover),            avg: fmtN(bm.assetTurnover),             good: ok('assetTurnover', r.assetTurnover, bm.assetTurnover) },
        ...
        { id: 'Stok Devir Süresi',      desc: 'Stokların kaç günde satıldığını gösterir. Düşük gün = stok hızlı dönüyor = nakite çevrim hızlı. Sektöre göre 30-90 gün arasında değişir.',                                    val: r.inventoryTurnoverDays != null ? fmtN(r.inventoryTurnoverDays, 0) + ' gün' : '—', avg: r.inventoryTurnoverDays != null ? fmtN(bm.inventoryDays, 0) + ' gün' : '—', good: ok('inventoryTurnoverDays', r.inventoryTurnoverDays, bm.inventoryDays) },
        { id: 'Alacak Tahsil Süresi',   desc: 'Müşteri alacaklarının kaç günde tahsil edildiği. Düşük = para hızlı geliyor. Yüksek oran nakit akışı sıkışıklığına yol açar.',                                                val: r.receivablesTurnoverDays != null ? fmtN(r.receivablesTurnoverDays, 0) + ' gün' : '—', avg: r.receivablesTurnoverDays != null ? fmtN(bm.receivablesDays, 0) + ' gün' : '—', good: ok('receivablesTurnoverDays', r.receivablesTurnoverDays, bm.receivablesDays) },
        { id: 'Borç Ödeme Süresi',      desc: 'Tedarikçi borçlarının kaç günde ödendiği. Yüksek = tedarikçi finansmanı uzun kullanılıyor (likidite için iyi, ama ilişki yönetimi önemli).',                                  val: r.payablesTurnoverDays != null ? fmtN(r.payablesTurnoverDays, 0) + ' gün' : '—', avg: fmtN(bm.payablesTurnoverDays, 0) + ' gün', good: ok('payablesTurnoverDays', r.payablesTurnoverDays, bm.payablesTurnoverDays) },
